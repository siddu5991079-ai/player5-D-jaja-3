...
async function startDirectStreaming() {
    console.log(`[*] Starting browser and FFmpeg on OS Level...`);
    ...
    const browserArgs = [
        ...
        '--kiosk', // Full screen to hide browser UI
    ...
    browser = await puppeteer.launch({
        ...
        defaultViewport: null, // Let kiosk mode take over
        ...
        if (target.type() === 'page') {
            try {
                ...
                if (newPage && newPage !== page) {
                    console.log(`[*] Adware tab detected! Closing...`);
                    await newPage.close();
                }
            } catch (e) { }
        }
    ...
    if (!targetFrame) throw new Error('No <video> element found.');

    // 🔊 AUDIO UNLOCKER + UI HIDER
    await targetFrame.evaluate(async () => {
        const style = document.createElement('style');
        style.innerHTML = `
            .jw-controls, .plyr__controls, .vjs-control-bar, button, .unmute-overlay {
    ...
    // 📡 FFMPEG OS-LEVEL BROADCAST (x11grab & pulseaudio)
    let ffmpegArgs = [];
...
setTimeout(async () => {
    console.log("\n[*] 5h 50m completed! Triggering next Github Action for overlap...");
